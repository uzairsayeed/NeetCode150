...
class Solution:
    ...
    # Day-07: 09/Aug/2026
    # Recogniton Pattern : Two sorted arrays need to be merged in-place, and one array has extra space at the end
        # → Think backward merge using three pointers.
    # Pattern: 3 Pointers (Back-merge)
    # Intuition:
        # The best way it to focus on the empty slots first
        # Since both the arrs are sorted in INCREASING order
        # => The RIGHT most slots should be occupied by the LARGEST available numbers
        # => 'i' = should be kept at nums1 LARGEST element 
            # => Since arrs are sorted in INCREASING order 
            # => i pointer position = m-1
        # => 'j' = should be kept at nums2 LARGEST element 
            # => Since arrs are sorted in INCREASING order 
            # => j pointer position = n-1
        # => 'k' = should be kept at last postion of nums1
            # => Since we are back-merging
        # After the pointers are fixed , we check for 2 cases
            # Case1: nums[i] < nums2[j] => The largest among them is from nums2 and it should occupy the RIGHT most position
            # Case2: nums1 element is LARGEST and it should occupy the RIGHT most poistion
            # In both the cases 'kth' positon is going to be consumed
        # Once we are done, there are 2 possibilities again
            # P1: Arr1 is exhausted first and arr2 is still not processed => j!=-1
                # In this case, we'll just add the left over elements of arr2 to arr1
            # P2: arr2 is exhausted but arr1 is not exhausted => i !=-1
