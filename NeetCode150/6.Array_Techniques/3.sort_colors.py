...
class Solution:
    ...
    # Day-07: 09/Aug/2026
    # Recognition Pattern: Need to partition an array containing only a few distinct values (0,1,2) in one pass, 
        # in-place, with O(1) extra space → Think Dutch National Flag and maintain region invariants.
    ...
    # TC = O(n), SC = O(1)
    # Mistakes Made:
        # Initially in the case where we swap the HIGH, I was incrementing mid as well
        # Which is not correct , since the SWAP with HIGH only means that 2 is at itrs right poisition
        # It doesnt mean MID also holds the correct element, iot should be considered as UNPROCESSED
        # Rating: 4.5/5
